"""
ResNet E1-D model
- 第一个7x7的卷积变为3x3，并且不进行下采样
- 下采样层使用ResNet-D的方式
- 总共下采样四次，图像大小变为原来的16分之一
- 适合STL-10数据集(96x96 -> 6x6)
"""
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1) if use_max_pool else nn.Identity()
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.Identity() if stride == 1 else nn.AvgPool2d(kernel_size=stride, stride=stride, ceil_mode=True, count_include_pad=False),
