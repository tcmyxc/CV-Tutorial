...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        return self.act_layer()(self.residual_function(x) + self.shortcut(x))
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100, act_layer=None, feat_dim=128, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        ...
        self.features = self.fc2(output)

        return logits

    def get_center_loss(self, y):
        x_cent = self.cls_cent[y]
        loss = torch.mean((self.features - x_cent) ** 2)
        return loss
